...
%Calculate J
J = (-1/m)*(sum(sum(y.*log(h)+(1-y).*log(1-h))))+(lambda/(2*m))*(sum(sum(Theta1(:,2:end).^2,2))+sum(sum(Theta2(:,2:end).^2,2)));


%BP Algorithm
s3=h-y;  % 5000 x 10
s2=(Theta2(:,2:end)'*s3').*sigmoidGradient(z2');   %25x5000
